...
export default function SecondaryOrderDelivery() {
  ...
  async function runBulk(orderIds, batchId, status, { closeBatchSheets = true } = {}) {
    setBusy(true)
    const { error } = await db.markOrdersDelivered({ orderIds, batchId, status, memberId: currentUser?.member_id })
    setBusy(false)
  ...
  async function savePartial() {
    ...
    const { error } = await db.markOrderPartiallyDelivered({
      orderId: partialOrder.id, batchId: partialOrder.batch_id, memberId: currentUser?.member_id, items,
    })
    setBusy(false)
    ...
    <div>
      ...
      {orders !== null && (isOwnView || drillMemberId) && (
        <Card>
          ...
          {batchRows.map(b => (
            <div key={b.batchId} onClick={() => isOwnView ? setProcessBatch(b) : setOrderDrillBatch(b)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', borderBottom: '1px solid #f3f4f6', cursor: 'pointer' }}>
              <div>
      ...
      {processBatch && (
        <Sheet title={processBatch.batchId} sub={`${processBatch.count} order(s) · ${F(processBatch.value)}`} onClose={() => setProcessBatch(null)}>
          ...
          <Btn v="pri" full disabled={busy} onClick={() => runBulk(processBatch.orders.map(o => o.id), processBatch.batchId, 'full')} style={{ marginBottom: 8 }}>
          ...
          <Btn v="bad" full disabled={busy} onClick={() => runBulk(processBatch.orders.map(o => o.id), processBatch.batchId, 'not_delivered')}>
            ...
            <div key={o.id} style={{ padding: '10px 4px', borderBottom: '1px solid #f3f4f6' }}>
              ...
              {isOwnView && (
                <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
                  <Btn sm v="pri" disabled={busy} onClick={() => runBulk([o.id], o.batch_id, 'full', { closeBatchSheets: false })}>Full</Btn>
                  <Btn sm disabled={busy} onClick={() => { setPartialOrder(o); setPartialItems({}) }}>Partial</Btn>
                  <Btn sm v="bad" disabled={busy} onClick={() => runBulk([o.id], o.batch_id, 'not_delivered', { closeBatchSheets: false })}>Not Delivered</Btn>
                </div>
              )}
            </div>
          ))}
      ...
      {partialOrder && (
        <Sheet title={partialOrder.id} sub="Enter returned qty per product (leave 0 for fully delivered items)" onClose={() => { setPartialOrder(null); setPartialItems({}) }} zIndex={320}>
          ...
          <Btn v="pri" full disabled={busy} onClick={savePartial} style={{ marginTop: 12 }}>Save Partial Delivery</Btn>
        </Sheet>
      )}
