...
function FieldRow({ fKey, label, fg, unit, dec, setDec, notes, setNotes }) {
  if (!fg) return null
...
function GoalReviewSheet({ member, param, goal, products, customers, categories, onAction, onClose }) {
  ...
  return (
    <Sheet title={`Review goals — ${member.name}`} sub="Approve or reject each field individually" onClose={onClose}>
      {param.enable_value && <FieldRow fKey="value" label="Sales value" fg={{ goal: g.value_goal, status: g.value_status, note: g.value_note }} unit="value" dec={dec} setDec={setDec} notes={notes} setNotes={setNotes} />}
      ...
      {param.enable_products && (param.sel_prods || []).map(id => {
        ...
        return <FieldRow key={id} fKey={`p_${id}`} label={`Product: ${p.name}`} fg={fg} unit={p.unit} dec={dec} setDec={setDec} notes={notes} setNotes={setNotes} />
      })}
